    ...
    public class ViewHolder extends RecyclerView.ViewHolder {
        ...
        private void setData(int pos, String ques, String a, String b, String c, String d, int selected, int correctAns){
            ...
            if(selected == -1){
                ...
                result.setTextColor(itemView.getContext().getResources().getColor(R.color.blackStart));
                setOptionColor(correctAns, R.color.greenAccent);
            }else{
                if(selected == correctAns){
                    result.setText("CORRECT");
                    result.setTextColor(itemView.getContext().getResources().getColor(R.color.greenAccent));
                    setOptionColor(selected, R.color.greenAccent);
                }else{
                    result.setText("WRONG");
                    result.setTextColor(itemView.getContext().getResources().getColor(R.color.redAccent));
                    setOptionColor(selected, R.color.redAccent);
                    setOptionColor(correctAns, R.color.greenAccent);
                }
            }
        }
